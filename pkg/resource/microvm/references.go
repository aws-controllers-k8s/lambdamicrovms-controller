...
func (rm *resourceManager) ClearResolvedReferences(res acktypes.AWSResource) acktypes.AWSResource {
	...
	if ko.Spec.ImageIdentifierRef != nil {
		ko.Spec.ImageIdentifier = nil
	}
...
func (rm *resourceManager) ResolveReferences(
	...
	if fieldHasReferences, err := rm.resolveReferenceForImageIdentifier(ctx, apiReader, ko); err != nil {
		return &resource{ko}, (resourceHasReferences || fieldHasReferences), err
	} else {
		resourceHasReferences = resourceHasReferences || fieldHasReferences
	}
...
// identifier field.
func validateReferenceFields(ko *svcapitypes.Microvm) error {
	...
	if ko.Spec.ImageIdentifierRef != nil && ko.Spec.ImageIdentifier != nil {
		return ackerr.ResourceReferenceAndIDNotSupportedFor("ImageIdentifier", "ImageIdentifierRef")
	}
	if ko.Spec.ImageIdentifierRef == nil && ko.Spec.ImageIdentifier == nil {
		return ackerr.ResourceReferenceOrIDRequiredFor("ImageIdentifier", "ImageIdentifierRef")
	}
	return nil
}
...
// resolveReferenceForImageIdentifier reads the resource referenced
// from ImageIdentifierRef field and sets the ImageIdentifier
// from referenced resource. Returns a boolean indicating whether a reference
// contains references, or an error
func (rm *resourceManager) resolveReferenceForImageIdentifier(
	ctx context.Context,
	apiReader client.Reader,
	ko *svcapitypes.Microvm,
) (hasReferences bool, err error) {
	if ko.Spec.ImageIdentifierRef != nil && ko.Spec.ImageIdentifierRef.From != nil {
		hasReferences = true
		arr := ko.Spec.ImageIdentifierRef.From
		if arr.Name == nil || *arr.Name == "" {
			return hasReferences, fmt.Errorf("provided resource reference is nil or empty: ImageIdentifierRef")
		}
		namespace, err := ackrt.ResolveCrossNamespaceReference(
			ctx,
			rm.cfg.EnableCrossNamespace,
		...
		obj := &svcapitypes.MicrovmImage{}
		if err := getReferencedResourceState_MicrovmImage(ctx, apiReader, obj, *arr.Name, namespace); err != nil {
			return hasReferences, err
		}
		ko.Spec.ImageIdentifier = (*string)(obj.Status.ACKResourceMetadata.ARN)
	}

	return hasReferences, nil
}

// getReferencedResourceState_MicrovmImage looks up whether a referenced resource
// exists and is in a ACK.ResourceSynced=True state. If the referenced resource does exist and is
// in a Synced state, returns nil, otherwise returns `ackerr.ResourceReferenceTerminalFor` or
// `ResourceReferenceNotSyncedFor` depending on if the resource is in a Terminal state.
func getReferencedResourceState_MicrovmImage(
	ctx context.Context,
	apiReader client.Reader,
	obj *svcapitypes.MicrovmImage,
	name string, // the Kubernetes name of the referenced resource
